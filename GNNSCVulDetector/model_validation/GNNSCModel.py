...
Options:
    ...
    --save-to FILE           File to save weights to.
    --restore FILE           File to restore weights from.
    --validation-set FILE    File to validate against when the model is restored. 
                             [default: train_data/reentrancy/valid.json]
    -t                       output
"""
...
class GNNSCModel(DetectModel):
    ...
    def process_raw_graphs(self, raw_data: Sequence[Any], is_training_data: bool) -> Any:
        ...
        count = len(raw_data)
        print("graph count: ", len(raw_data))
...
def main():
    ...
    validating_saved_model = bool(args.get('--restore'))
    
    if validating_saved_model:
        # re-initialize validation set
        validation_set = dict(args).get('--validation-set', 'train_data/reentrancy/valid.json' )
        # import pdb; pdb.set_trace()
        model.valid_data, model.valid_num_graph = model.load_data(validation_set, is_training_data=False)
    
    model.train()

    if not validating_saved_model:
        model.save_model(args.get('--save-to', 'gnnsc_reentrancy.bin'))
