    ...
    def train(self):
        ...
        with self.graph.as_default():
            if self.args.get('--restore') is not None:
                _, valid_accs, _, _ = self.run_epoch("Resumed (validation)", self.valid_data, self.params['num_epochs'], False)
                best_val_acc = np.sum(valid_accs)
                best_val_acc_epoch = 0
                print("\r\x1b[KInitial cum. val. acc: %.5f" % best_val_acc)
                exit()
            else:
